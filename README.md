...
The request includes the page URL (`_url`, without query parameters or fragments) so FormSubmit can identify the form when browser referrer information is unavailable. Rejections display FormSubmit's message as plain text when provided, including on HTTP error responses. If submission fails, check that message and the activation email before retrying; see [FormSubmit troubleshooting](https://formsubmit.co/help).

The Careers page uses FormSubmit's standard multipart endpoint so applicants can attach a required résumé and an optional cover letter. It accepts PDF, DOC, and DOCX files within FormSubmit's combined 10 MB upload limit and returns successful applicants to `/careers?submitted=true`. The form may require a separate one-time FormSubmit activation email after its first live submission.
...
Vercel rewrites each secondary route to its generated HTML document. Deploy the complete `build` directory using `npm run build`, not just `react-scripts build`, which skips the npm lifecycle steps. For a local production preview use `npx serve build -l 3001` without `-s`; the SPA fallback would serve Home's HTML at every URL and cause hydration mismatches. Other hosts must map `/about`, `/services`, `/vendors`, `/projects`, `/contacts`, and `/careers` to the corresponding `.html` files. `npm start` remains a client-rendered development server, not a production performance preview.
