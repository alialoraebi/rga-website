...
async function prerender() {
  ...
  const template = await fs.readFile(path.join(root, 'build/index.html'), 'utf8');
  const assets = JSON.parse(await fs.readFile(path.join(root, 'build/asset-manifest.json'), 'utf8'));
  for (const [pathname, chunk, title] of routes) {
    ...
    const chunkUrl = Object.values(assets.files).find(url => url.includes(`/js/${chunk}.`) && url.endsWith('.js'));
    ...
    assert.equal(document.querySelectorAll('main h1').length, 1, `Missing initial content for ${pathname}`);
    ...
    if (pathname === '/services') {
      assert.equal(document.querySelector('main img').getAttribute('loading'), 'eager');
      assert.equal(document.querySelector('main img').getAttribute('fetchpriority'), 'high');
    ...
    dom.window.close();
    console.log(`Generated and verified ${pathname}: initial content, responsive images, and ${chunk} chunk preload.`);
  }
}
